...
import { 
  ...
  FileDown, X, Loader2, FileText, ChevronDown, Upload, Paintbrush, Save
} from 'lucide-react';
...
const FontSize = Extension.create({
  ...
  addAttributes() {
    return {
      fontSize: {
        default: null,
        parseHTML: (element: { style: { fontSize: any; }; }) => element.style.fontSize,
...
const DocumentGeneratorContent = () => {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'h-full outline-none prose prose-sm sm:prose max-w-none focus:outline-none',
        spellcheck: 'false',
      },
      handleDOMEvents: {
  ...
  const debugMarks = () => {
    if (editor) {
      ...
      console.log("Selected text:", editor.state.selection.content().content.firstChild?.text);
  ...
  const handleCustomFontSizeSubmit = (e: React.KeyboardEvent<HTMLInputElement>) => {
  ...
  const generateImage = async () => {
    ...
    try {
      ...
      if (data.imageUrl) {
        ...
        if (data.fallback) {
          alert("Using a placeholder image as AI image generation failed. You can try again or use this placeholder."); 
        }
      }
    } catch (error) {
      console.error('Error generating image:', error);
      alert("Failed to generate image. Please try again with a different description.");
    } finally {
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" size="sm" className="gap-1 min-w-[120px]">
              <span className="truncate">{fontFamilies.find(f => f.value === currentFontFamily)?.name || 'Font'}</span>
              <ChevronDown size={14} />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent>
                ...
                {generatedImageUrl && (
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex justify-between">
                      <Button variant="outline" size="sm" onClick={regenerateImage}>
                        Regenerate
                      </Button>
