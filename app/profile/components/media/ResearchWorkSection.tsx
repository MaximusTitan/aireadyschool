...
export default function ResearchWorkSection() {
  ...
  const handleResearchWorkUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleVaultDocumentSelect = async (document: VaultItem) => {
    setIsUploading(true)
    try {
      await uploadVaultDocument(document)
    } catch (error) {
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from("research-works").upload(filePath, file)
  ...
  const handleResearchWorkDelete = async (id: string) => {
    ...
    try {
      ...
      if (filePath) {
        await supabase.storage.from("research-works").remove([`${user.email}/${filePath}`])
      }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              ...
              {editingResearchWork?.id === work.id ? (
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    className="flex-grow"
                  />
                  <Button size="icon" variant="ghost" onClick={handleResearchWorkTitleSave}>
                    <Check className="h-4 w-4" />
                  </Button>
                  <Button size="icon" variant="ghost" onClick={() => setEditingResearchWork(null)}>
                    <X className="h-4 w-4" />
                  </Button>
                </div>
              ) : (
                <div className="flex items-center justify-between">
                  ...
                  <div>
                    <Button
                      size="icon"
                      variant="ghost"
                      onClick={() => handleResearchWorkTitleEdit(work.id, work.title)}
                    >
                      <Edit2 className="h-4 w-4" />
                    </Button>
                    <Button size="icon" variant="ghost" onClick={() => handleResearchWorkDelete(work.id)}>
                      <X className="h-4 w-4" />
                    </Button>
                  </div>
                </div>
                  ...
                  <DialogContent className="max-w-4xl h-[80vh]">
                    ...
                    <div className="mt-4 h-full">
                      {work.file_url.endsWith(".pdf") ? (
                        <object data={work.file_url} type="application/pdf" width="100%" height="100%">
                          <p>
        ...
        <div className="aspect-video bg-gray-100 rounded-lg border-2 border-dashed border-gray-300 hover:border-gray-400 transition-colors flex flex-col items-center justify-center cursor-pointer p-4">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => researchWorkInputRef.current?.click()} disabled={isUploading}>
              {isUploading ? (
                <>
