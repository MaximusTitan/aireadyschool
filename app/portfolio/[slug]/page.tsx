...
export default async function PublicPortfolio({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: studentData, error: studentError } = await supabase
    .from("students")
    .select("*")
    .eq("profile_slug", slug) // Using awaited param
    .single();
  ...
  const { data: researchWorks, error: researchWorksError } = await supabase
    .from("research_works")
    .select("*")
    .eq("student_email", studentData.student_email)
                        ...
                        <CardContent className="flex flex-col aspect-square p-4">
                          ...
                          <div className="flex justify-between items-start">
                            <div className="flex flex-col min-w-0 flex-1 mr-2">
                              <h3 className="font-semibold text-sm truncate" title={artwork.title}>
                                {artwork.title}
                              </h3>
                            ...
                            <a
                              ...
                              className="shrink-0 px-3 py-2 bg-rose-500 text-white text-xs rounded hover:bg-rose-600 transition-colors"
                            >
                              View
                            </a>
                          </div>
                    ...
                    <div className="p-1">
                      <Card>
                        <CardContent className="flex flex-col aspect-square p-4">
                          ...
                          <div className="flex justify-between items-start">
                            <div className="flex flex-col min-w-0 flex-1 mr-2">
                              <h3 className="font-semibold text-sm truncate" title={work.title}>
                                {work.title}
                              </h3>
                              <p className="text-gray-500 text-xs mt-1">
                      ...
                      <div className="p-1">
                        <Card>
                          <CardContent className="flex flex-col aspect-square p-4">
                            ...
                            <div className="flex justify-between items-start">
                              <div className="flex flex-col min-w-0 flex-1 mr-2">
                                <h3 className="font-semibold text-sm truncate" title={presentation.title}>
                                  {presentation.title}
                                </h3>
                                <p className="text-gray-500 text-xs mt-1">
                    ...
                    <div className="p-1">
                      <Card>
                        <CardContent className="flex flex-col aspect-square p-4">
                          ...
                          <div className="flex justify-between items-start">
                            <div className="flex flex-col min-w-0 flex-1 mr-2">
                              <h3 className="font-semibold text-sm truncate" title={app.name}>
                                {app.name}
                              </h3>
                              <p className="text-gray-500 text-xs mt-1">
