...
import { useChatThread } from "@/app/tools/buddy/hooks/useChatThread";
import { ThreadList } from "./components/thread-list";
import { useRouter } from "next/navigation";
import { Message } from "ai";
import { ChatMessage } from "@/types/chat";
import { useLanguageSettings } from "@/app/tools/buddy/hooks/useLanguageSettings";
import { useTools } from "@/app/tools/buddy/hooks/useTools";
...
export default function Page() {
  ...
  const handleThreadSelect = async (selectedThreadId: string) => {
    try {
      ...
      router.push(`/tools/buddy?thread=${selectedThreadId}`);
    } catch (error) {
  ...
  const handleDeleteThread = async (threadId: string) => {
    if (confirm("Are you sure you want to delete this chat?")) {
      try {
        ...
        router.replace("/tools/buddy", { scroll: false });
      } catch (error) {
  ...
  const handleNewThread = async () => {
    try {
      ...
      setMessages([]);
      router.push(`/tools/buddy?thread=${newThreadId}`);
    } catch (error) {
  ...
  const processDirectCommand = async (command: string) => {
    ...
    if (!threadId) {
      ...
      await router.push(`/tools/buddy?thread=${newThreadId}`);
    }
  ...
  const handleMessageSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!activeThreadId) {
        ...
        await router.push(`/tools/buddy?thread=${activeThreadId}`);
        await new Promise((resolve) => setTimeout(resolve, 100));
      }
