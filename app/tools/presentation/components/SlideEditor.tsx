...
export default function SlideEditor({ slide, onUpdate }: SlideEditorProps) {
  ...
  const handleRegenerateImage = async (imageType: 'main' | 'right' | 'additional', index?: number) => {
    setIsRegenerating(true)
    try {
      const newImage = await regenerateImage(slide.title + (imageType === 'right' ? ' alternative' : ''), imageType)
      if (imageType === 'main') {
          ...
          <div className="grid grid-cols-2 gap-8">
            <div className="space-y-4">
              <div className="relative aspect-video bg-muted rounded-lg overflow-hidden">
                ...
                <RegenerateButton onClick={() => handleRegenerateImage('main')} disabled={isRegenerating} />
              </div>
              <Textarea
            ...
            <div className="space-y-4">
              <div className="relative aspect-video bg-muted rounded-lg overflow-hidden">
                <img src={slide.rightImage || slide.image} alt={slide.title} className="w-full h-full object-cover" />
                <RegenerateButton onClick={() => handleRegenerateImage('right')} disabled={isRegenerating} />
              </div>
              <Textarea
        ...
        return (
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              {[slide.image, slide.rightImage, ...(slide.additionalImages || [])].slice(0, 4).map((img, index) => (
                <div key={index} className="relative aspect-square bg-muted rounded-lg overflow-hidden">
                  <img src={img} alt={`${slide.title} ${index + 1}`} className="w-full h-full object-cover" />
                  <RegenerateButton
                    onClick={() => handleRegenerateImage(index === 0 ? 'main' : index === 1 ? 'right' : 'additional', index - 2)}
    ...
    <Card className="mb-4">
      <CardContent className="p-6">
        <div className="flex items-center gap-4 mb-8">
          ...
          />
          <Select value={slide.layout} onValueChange={(value: SlideLayout) => handleChange('layout', value)}>
