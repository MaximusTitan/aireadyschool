...
export function DocumentVaultSelector({ onSelect }: DocumentVaultSelectorProps) {
  ...
  const GridViewItem = ({ item }: { item: VaultItem }) => (
    <Card className="w-40 h-40 relative group cursor-pointer" onClick={() => onSelect(item)}>
      ...
      <DialogContent className="sm:max-w-[725px]">
        ...
        <div className="mt-4">
          ...
          {isLoading ? (
            ...
            <>
              {items.length === 0 ? (
                <p className="text-gray-500">No items found in your document vault.</p>
              ) : viewType === "list" ? (
